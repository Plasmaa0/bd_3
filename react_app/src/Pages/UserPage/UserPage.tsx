...
export function UserPage() {
    ...
    if (isLoading) return (
        <div>
            <Typography.Title>User page: {user} is loading</Typography.Title>
            {/*<Typography.Text>Loading...</Typography.Text>*/}
            <ClockCircleTwoTone spin twoToneColor={"lime"} style={{fontSize: '50px'}}/>
        </div>);
    if (isFetching) return (
        <div>
            <Typography.Title>User page: {user} is fetching</Typography.Title>
            {/*<Typography.Text>Fetching...</Typography.Text>*/}
            <ClockCircleTwoTone spin twoToneColor={"lime"} style={{fontSize: '50px'}}/>
        </div>);
    if (error) return (
        <div>
            <Typography.Title>User page: {user} loaded with error</Typography.Title>
            {/*// @ts-ignore*/}
    ...
    const displayedUser = user || "unknown";
                ...
                <Col>
                    <Collapse>
                        <Collapse.Panel header="Add project" key="1">
                            <AddProjectForm existingProjects={displayedData} user={displayedUser}
                                            setNeedToRefetch={setNeedToRefetch}/>
                        </Collapse.Panel>
                    </Collapse>
                </Col>
            </Row>
            <Table pagination={{hideOnSinglePage: true}} columns={columns} dataSource={displayedData}></Table>
        </>
    )
    // return (
    //     <div>
    //         <h1>
    //             User page: {user} <br/>
    //             user {user === null ? " is null" : " is not null"} <br/>
    //             data {data ? " is not null" : " is null"}
    //         </h1>
    //     </div>
    // )
}